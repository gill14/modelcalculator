...
class AtomizationModel:
    ...
    def _dsc_dv(self, dv: int, dv_key: str) -> str:
        r = self.ref_nozzles
        dsc = ""
        if dv > 0:
            for category in r.keys():
    ...
    def volfracs(self):
        return [self.dv01(), self.dv05(), self.dv09()]
    ...
    def dsc_dv01(self, dv01=None):
        if dv01 is None:
            dv01 = self.dv01()
        return self._dsc_dv(dv=dv01, dv_key="DV01")

    def dsc_dv05(self, dv05=None):
        if dv05 is None:
            dv05 = self.dv05()
        return self._dsc_dv(dv=dv05, dv_key="DV05")

    def dsc_dv09(self, dv09=None):
        if dv09 is None:
            dv09 = self.dv09()
        return self._dsc_dv(dv=dv09, dv_key="DV09")
    ...
    def dsc_name(self, dsc=None):
        return self._dsc_name(dsc=self._dsc())
    
    def dsc_name_dv01(self, dsc=None):
        if dsc is None:
            dsc = self.dsc_dv01()
        return self._dsc_name(dsc)
    
    def dsc_name_dv05(self, dsc=None):
        if dsc is None:
            dsc = self.dsc_dv05()
        return self._dsc_name(dsc)
    
    def dsc_name_dv09(self, dsc=None):
        if dsc is None:
            dsc = self.dsc_dv09()
        return self._dsc_name(dsc)
    ...
    def get_nozzles() -> list[str]:
        return AtomizationModel.nozzles_extended

    def get_orifices_for_nozzle(nozzle: str) -> list[str]:
        return AtomizationModel._params_for_nozzle(AtomizationModel, nozzle, "Orifice")

    def get_deflections_for_nozzle(nozzle: str) -> list[str]:
        return AtomizationModel._params_for_nozzle(AtomizationModel, nozzle, "Angle")
